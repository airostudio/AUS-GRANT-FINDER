...
/**
 * Western Australia Government Grants & Tenders Client
 *
 * Data sources:
 * 1. Tenders WA: https://www.tenders.wa.gov.au/ (the main WA tenders portal)
 * 2. Grants WA: https://www.wa.gov.au/government/grants
 * 3. data.wa.gov.au (CKAN): https://data.wa.gov.au/api/3/action/
 * 4. Lotterywest Grants: https://www.lotterywest.wa.gov.au/grants
 * 5. Healthway Grants: https://www.healthway.wa.gov.au/grants/
 */
export class WAGrantsClient implements APIClient {
  name = 'WA Grants';
  private tendersUrl: string;
  private ckanUrl: string;

  constructor() {
    this.tendersUrl = 'https://www.tenders.wa.gov.au/api';
    this.ckanUrl = 'https://data.wa.gov.au/api/3';
  }
  ...
  async fetchOpportunities(params: SearchParams): Promise<Opportunity[]> {
    try {
      if (params.state && params.state !== 'wa') return [];
  ...
  private async fetchTenders(params: SearchParams): Promise<Opportunity[]> {
    try {
      // Try the Tenders WA open contract opportunities endpoint
      const response = await fetch(`${this.tendersUrl}/opportunities?status=open&limit=100`, {
  ...
  private transformTenders(data: any, params: SearchParams): Opportunity[] {
    const items = data?.tenders || data?.opportunities || data?.results || data?.data || [];
    ...
    for (const item of items) {
      ...
      const title = item.title || item.description || 'WA Government Tender';
      ...
      opportunities.push({
        ...
        amount,
        description: item.description || `WA Government tender. Agency: ${item.agency || 'Western Australian Government'}`,
        jurisdiction: 'state',
        state: 'wa',
        openDate: openDate || new Date().toISOString().split('T')[0],
        closeDate: effectiveCloseDate,
        url: item.url || `https://www.tenders.wa.gov.au/`,
        status,
      });
    }
  ...
  private transformCkanResponse(data: any, params: SearchParams): Opportunity[] {
    ...
    for (const pkg of data.result.results) {
      ...
      opportunities.push({
        id: `wa-ckan-${pkg.id || pkg.name}`,
        title: pkg.title || 'WA Government Grant',
        type: 'grant',
        category: 'community',
        amount: null,
        description: pkg.notes || `Western Australian Government data resource. Updated: ${modifiedDate}.`,
        jurisdiction: 'state',
        state: 'wa',
        openDate: modifiedDate || new Date().toISOString().split('T')[0],
        closeDate,
        url: `https://data.wa.gov.au/dataset/${pkg.name || pkg.id}`,
        status: 'open',
      });
  ...
  private mapCategory(title: string): string {
    ...
    if (t.includes('maintenance') || t.includes('repair')) return 'maintenance';
